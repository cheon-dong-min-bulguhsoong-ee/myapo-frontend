...
export default function DisputesPage() {
  ...
  return (
    <div className="flex flex-col h-full bg-canvas">
      <AppBar title="이의 신청" />
      <PageHeader size="hero" title="이의 신청 내역" subtitle="신고하신 건의 처리 상태를 확인하세요" />
      ...
      <main className="flex-1 overflow-y-auto px-5 pb-6 space-y-3">
        {mockDisputes.length === 0 ? (
          ...
          mockDisputes.map(d => (
            <Card key={d.id} clickable className="border border-hairline" onClick={() => router.push(`/disputes/${d.id}`)}>
              <div className="flex justify-between items-start">
                <div className="space-y-1.5">
                  <span className="ds-headline">{d.documentType}</span>
                  <div className="flex gap-2 items-center">
                    <Pill variant={disputeStatusMap[d.status].variant}>{disputeStatusMap[d.status].label}</Pill>
                    <span className="text-[13px] text-ink-muted">{d.stage} 단계</span>
                  </div>
                  <p className="text-[13px] text-ink-muted">{d.createdAt}</p>
                </div>
